  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate("/students")} className="text-slate-500 hover:text-[var(--brand-primary)] dark:hover:text-white">
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <h1 className="text-2xl font-bold tracking-tight text-[var(--brand-primary)] dark:text-white">Student Profile</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="col-span-1 border-none shadow-lg dark:bg-[#1e293b] overflow-hidden relative">
          ...
          <CardContent className="pt-16 pb-6 px-6 relative z-10 flex flex-col items-center text-center">
            ...
            <h2 className="text-2xl font-bold text-[var(--brand-primary)] dark:text-white">{student.name}</h2>
            <div className="flex items-center justify-center gap-2 mt-2">
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[var(--brand-primary)]/10 text-[var(--brand-primary)] dark:bg-[var(--brand-primary)]/30 dark:text-[#60a5fa] border border-[var(--brand-primary)]/20">
                {student.grade}
              </span>
            ...
            <div className="flex gap-2 mt-6 w-full">
              ...
              <Button onClick={() => setShowReport(true)} variant="outline" className="flex-1 border-[var(--brand-primary)]/20 text-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/5 dark:text-white dark:border-white/20"><FileText className="mr-2 h-4 w-4" /> Report</Button>
            </div>
                  ...
                  {[
                    ...
                    <div key={i} className="flex justify-between items-center p-3 rounded-lg border border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded bg-[var(--brand-primary)]/10 flex items-center justify-center text-[var(--brand-primary)] dark:text-[#60a5fa] font-bold text-xs">
                          {item.subject.substring(0, 3).toUpperCase()}
                        </div>
                        <h4 className="font-semibold text-slate-800 dark:text-slate-200">{item.subject}</h4>
                      </div>
                      <div className="text-right">
                        <span className="font-bold text-[var(--brand-secondary)] dark:text-[#4ade80]">{item.grade}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Card className="border shadow-sm dark:bg-[#1e293b]">
              ...
              <CardContent>
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  <div className="h-16 w-16 rounded-full bg-green-100 dark:bg-[var(--brand-secondary)]/20 flex items-center justify-center mb-4">
                  ...
                  <Button onClick={() => navigate("/fees")} variant="outline" className="mt-6 border-[var(--brand-primary)]/20 text-[var(--brand-primary)] dark:text-white hover:bg-[var(--brand-primary)]/5">
                    View Fee History
                  </Button>
                </div>
              </CardContent>
      ...
      <Dialog open={showReport} onOpenChange={setShowReport}>
        <DialogContent className="sm:max-w-[600px] p-0 dark:bg-[#1e293b] border-none overflow-hidden">
          <div className="bg-[var(--brand-primary)] p-6 text-center text-white relative flex flex-col items-center">
            ...
            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2">
              <Avatar className="h-20 w-20 border-4 border-white dark:border-[#1e293b] shadow-xl">
                <AvatarImage src={`https://i.pravatar.cc/150?u=${student.name}`} />
                <AvatarFallback className="bg-[var(--brand-secondary)]">{student.name.charAt(0)}</AvatarFallback>
              </Avatar>
            </div>
          </div>
          ...
          <div className="p-6 pt-14 space-y-6">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  <tr>
                    ...
                    <td className="px-4 py-3 text-right font-bold text-[var(--brand-secondary)] dark:text-[#4ade80]">A+</td>
                  </tr>
                  <tr>
            ...
            <div className="flex justify-between items-center bg-slate-50 dark:bg-white/5 p-4 rounded-lg">
              <div>
                ...
                <p className="text-2xl font-bold text-[var(--brand-primary)] dark:text-white">3.85<span className="text-sm text-muted-foreground font-normal">/4.0</span></p>
              </div>
              <div className="text-right">
                <p className="text-sm text-muted-foreground">Attendance</p>
                <p className="text-xl font-bold text-[var(--brand-secondary)] dark:text-[#4ade80]">96%</p>
              </div>
            </div>

            <div className="flex justify-end gap-3 pt-2">
              <Button variant="outline" className="border-white/10 dark:text-white" onClick={() => setShowReport(false)}>Close</Button>
              <Button onClick={() => toast.promise(new Promise(r => setTimeout(r, 1000)), { loading: 'Downloading...', success: 'Report Downloaded!', error: 'Error' })} className="bg-[var(--brand-primary)] hover:bg-[var(--brand-primary-light)] text-white">
